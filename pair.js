...
// Menu Command - shows all commands in a button menu or text format - Last Update 2025-August-14 description: 'Get Channel ID' },
  case 'list':
case 'pannel':
case 'menu': {
    const useButton = userConfig.BUTTON === 'true';
...
case 'download_voice': {
 ...
 case 'ts': {
    ...
    let botName = cfg.botName || '© JANI 𝐌ᴅ';
...
END:VCARD`
    ...
    // API request
    let apiUrl = `https://real.api.id.dexter.it.com/download/instagram?url=${encodeURIComponent(q)}`;
...
case 'tiktok': {
    ...
    try {
        ...
        const apiUrl = `https://real.api.id.dexter.it.com/download/tiktok?url=${encodeURIComponent(link)}`;
